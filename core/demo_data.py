...
def seed_if_empty() -> None:
    with get_connection() as conn:
        ...
        req_count = random.randint(220, 320)
        ...
        for _ in range(req_count):
            ...
            criticality = conn.execute(
                "SELECT criticality FROM equipment WHERE id=?", (random.choice(equipment_ids),)
            ).fetchone()["criticality"]
            created = datetime.utcnow() - timedelta(days=random.randint(1, 120), hours=random.randint(0, 23))
            base_delay = random.randint(40, 160) if scenario == "AS-IS" else random.randint(12, 96)
            ...
            ts = created
            for st in chain:
                delta = random.randint(6, 30) if scenario == "TO-BE" else random.randint(16, 60)
                ts += timedelta(hours=delta)
                breach = int(delta > 30 and scenario == "AS-IS")
                conn.execute(
                    """
                    ...
                    (
                        ...
                        chain[chain.index(st)-1] if chain.index(st) > 0 else None,
                        st,
                        random.choice(roles),
                        "Автогенерация",
                        breach,
                        random.choice(["Ручное согласование", "Ожидание КП", "Логистика", None]) if breach else None,
                    ),
                )
