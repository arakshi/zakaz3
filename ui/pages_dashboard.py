...
def render_dashboard_page(scenario: str) -> None:
    ...
    with col1:
        ...
        if not kpi["top_delays"].empty:
            ...
            if not fallback.empty:
                fig = px.bar(fallback, x="delay_reason", y="count", color="count", color_continuous_scale="Blues")
                st.plotly_chart(fig, use_container_width=True)
            else:
    ...
    if not req_cmp.empty:
        ...
        fig = px.bar(cmp_df, x="scenario", y="cycle_h", color="scenario", text=cmp_df["cycle_h"].round(1))
