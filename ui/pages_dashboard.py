...
def _apply_plot_contrast(fig):
    fig.update_layout(
        template="plotly_white",
        font={"color": DARK_TEXT},
        paper_bgcolor=LIGHT_BG,
        plot_bgcolor=LIGHT_BG,
        legend={"font": {"color": DARK_TEXT}, "title_font": {"color": DARK_TEXT}},
        title_font={"color": DARK_TEXT},
    )
    fig.update_xaxes(
    ...
    # Принудительно задаем цвет текста трейсов (важно для подписей на барах/секторах)
    fig.update_traces(textfont_color=DARK_TEXT)
    return fig
...
def render_dashboard_page(scenario: str) -> None:
    ...
    with col1:
        ...
        if not kpi["top_delays"].empty:
            ...
            st.plotly_chart(_apply_plot_contrast(fig), use_container_width=True)
        else:
            fallback = (
    ...
    with col2:
        ...
        if not kpi["queue_by_role"].empty:
            ...
            fig.update_traces(textposition="inside", textinfo="percent+label", insidetextfont_color="#111827")
            st.plotly_chart(_apply_plot_contrast(fig), use_container_width=True)
        else:
            st.info("Нет данных")
    ...
    if not req_cmp.empty:
        ...
        fig = px.bar(
            ...
            y="cycle_h",
            color="scenario",
            text=cmp_df["cycle_h"].round(1),
            color_discrete_map={"AS-IS": "#ef4444", "TO-BE": "#10b981"},
        )
        ...
        if not delays_cmp.empty:
            ...
            st.plotly_chart(_apply_plot_contrast(fig_delay), use_container_width=True)
